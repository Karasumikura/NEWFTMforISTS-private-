...
class VariableEmbeddingQwen(nn.Module):
    def __init__(self, c_in: int, d_model: int):
        super().__init__()
        self.emb = nn.Embedding(c_in, d_model)

    def forward(self, B: int, L: int, D: int, device: torch.device) -> torch.Tensor:
        idx = torch.arange(D, device=device).view(1, 1, D).expand(B, L, D)
        return self.emb(idx)  # (B,L,D,H)


# ===================== CT-RoPE 注入（仅 Qwen2.5） =====================

def inject_ctr_rope_for_qwen25(core_model, verbose: bool = True):
    """
    遍历 core_model.layers，包装每个 layer.self_attn.forward，使其支持 timestamps (连续时间戳)。
    仅适配 trust_remote_code 加载的 Qwen2.5。
    """
    if not hasattr(core_model, "layers"):
        raise ValueError("Loaded Qwen2.5 core model不含 layers 属性，请确认模型结构。")

    layers = core_model.layers
    if verbose:
        print(f"[CT-RoPE] Qwen2.5 层数: {len(layers)}")

    def build_forward(old_attn):
        # 读取注意力投影层
        ...
        if any(x is None for x in [q_proj, k_proj, v_proj, o_proj]):
            raise ValueError("自定义注入失败：注意力层缺少 q_proj/k_proj/v_proj/o_proj")

        cfg = getattr(old_attn, "config", None)
        if cfg is None:
            raise ValueError("注意力层缺少 config 属性。")
        ...
        def new_forward(
            ...
            **kwargs,
            ...
            k = k.view(bsz, q_len, num_kv, head_dim).transpose(1, 2)         # (B,H_kv,L,hd)
    ...
    for i, layer in enumerate(layers):
        if hasattr(layer, "self_attn"):
            ...
            wrapped_forward = build_forward(orig_attn)
            layer.self_attn.forward = wrapped_forward
            if verbose:
                print(f"[CT-RoPE] Layer {i} attention patched: {type(orig_attn)}")
...
def apply_lora(args, core_model):
...
class istsplm_forecast(nn.Module):
    def __init__(self, args):
        ...
        for _ in range(self.n_plm_layers):
            ...
            core = inject_ctr_rope_for_qwen25(core, verbose=False)
            if self.use_lora:
                core = apply_lora(self.args, core)
            gpts.append(core)
    ...
    def forecasting(self, batch_dict, n_vars_to_predict=None):
        ...
        # Variable PLM 输入
        var_emb = self.var_emb.weight.unsqueeze(0).expand(B, D, -1)
