...
class istsplm_forecast(nn.Module):
    ...
    def _inject_ctr_rope(self, core_model, verbose: bool = True):
        ...
        try:
            ...
            raise RuntimeError("无法导入 apply_rotary_pos_emb，请升级 transformers 版本。")
    ...
    def forecasting(self, batch_dict, n_vars_to_predict=None):
        ...
        tp_to_predict = batch_dict["tp_to_predict"].to(self.device)
        ...
        out_val = out_time[:, self.prompt_len:]
        denom = mask_pool.sum(dim=1, keepdim=True) + 1e-8
        pooled = (out_val * mask_pool).sum(dim=1) / denom.squeeze(1)
        ...
        forecast = regression_value.unsqueeze(1).expand(-1, L_pred, -1)
